...
import static net.chessbuddies.engine.lookup.Coordinates.*;
import static net.chessbuddies.engine.lookup.Masks.*;
import static net.chessbuddies.engine.lookup.PieceTables.*;
import static net.chessbuddies.engine.lookup.Pieces.*;
 ...
 */
public class Board {
	private static byte[] emptyBoard = new byte[128];
	private static final String VALID_FILES = "RNBQKPrnbqkp12345678";
	private static byte[] PIECES = new byte[] { ROOK, KNIGHT, BISHOP, QUEEN, KING, PAWN, BROOK, BKNIGHT, BBISHOP, BQUEEN, BKING, BPAWN };

	static {
		Arrays.fill(emptyBoard, EMPTY);
	}
	...
	/**
	 * Initialise and create the board to contain chess pieces arranged in an order
	 * such that the resulting positions represent a valid chess starting position.
	 */
	public Board() {
		this.squares = new byte[] { ROOK, KNIGHT, BISHOP, QUEEN, KING, BISHOP, KNIGHT, ROOK, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, PAWN, PAWN, PAWN, PAWN, PAWN,
				PAWN, PAWN, PAWN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY,
				...
				EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY,
				EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, BPAWN, BPAWN, BPAWN, BPAWN, BPAWN, BPAWN, BPAWN, BPAWN, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, BROOK, BKNIGHT,
				BBISHOP, BQUEEN, BKING, BBISHOP, BKNIGHT, BROOK, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY };
	...
	public Board(String fen) {
		...
		if (fields.length != 6) {
			throw new IllegalArgumentException(String.format("Not a valid FEN '%s'. All 6 fields must be set ", fen));
		}
		// first field is board occupancy, separated by '/' for each rank
		String[] ranks = StringUtils.split(fields[0], '/');
		if (ranks.length != 8) {
			throw new IllegalArgumentException(String.format("Not a valid FEN '%s'. All 8 ranks must be set ", fen));
		}
		for (int j = 0; j < 8; j++) {
			String rankStr = ranks[j];
			int rank=7-j;
			if (StringUtils.isBlank(rankStr) || rankStr.length() > 8) {
				throw new IllegalArgumentException(String.format("Not a valid FEN '%s'. Rank %d is invalid ", fen, rank + 1));
			}
			int file=-1;
			for (int i = 0; i < rankStr.length(); i++) {
				file++;
				char c = rankStr.charAt(i);
				int idx = VALID_FILES.indexOf(c);
		//		System.out.println("char="+c+", idx="+idx);
				if (idx == -1) {
					throw new IllegalArgumentException(String.format("Not a valid FEN '%s'. Invalid character at Rank %d", fen, rank + 1));
				}
				if (idx > 11) { // a number
					file +=(c-49);
				} else {
					int box = ((rank) * 16) + (file);
					byte piece = PIECES[idx];
					this.squares[box] = piece;
					if (piece == KING || piece == BKING) {
						if (piece == KING) {
							this.whiteKingPosition = box;
						} else {
							this.blackKingPosition = box;
						}
					}
				}
			}
		}
		this.turnColour = fields[1].equals("w") ? WHITE : BLACK;
		printBoard();
		markMoved();
		this.previousMove = null;
		this.validMoves = generateValidMoves();
	...
	private void markMoved() {
		for (int rank = 0; rank < 8; rank++) {
			int start = rank * 16 ;
			for (int file = start; file <= start+7; file++) {
				byte pieceType = pieceTypeAt(file);
				if (pieceType == EMPTY) {
					continue;
				}
				if ( pieceType == PAWN && rank != 1) {
					//white pawn
					setMovementBit(file);
				}else if (pieceType == BPAWN && rank != 7) {
					//black pawn moved
					setMovementBit(file);
				}else if (pieceType == KING && file != 4) {
					setMovementBit(file);
				}else if (pieceType == BKING && file != 116) {
					setMovementBit(file);
				}else if (pieceType == QUEEN && file != 3) {
					setMovementBit(file);
				}else if (pieceType == BQUEEN && file != 115) {
					setMovementBit(file);
				}else if (pieceType == BISHOP && (file != 2 || file != 5)) {
					setMovementBit(file);
				}else if (pieceType == BBISHOP && (file != 114 || file != 117)) {
					setMovementBit(file);
				}else if (pieceType == KNIGHT && (file != 1 || file != 6)) {
					setMovementBit(file);
				}else if (pieceType == BKNIGHT && (file != 113 || file != 118)) {
					setMovementBit(file);
				}else if (pieceType == ROOK && (file != 0 || file != 7)) {
					setMovementBit(file);
				}else if (pieceType == BROOK && (file != 112 || file != 119)) {
					setMovementBit(file);
				}
			}
		}
	...
	private void printBoard() {
		...
		for (int rank = 0; rank < 8; rank++) {
			...
			int start = rank * 16 ;
			for (int file = start; file <= start+7; file++) {
				byte piece = pieceAt(file);
				String p=" ";
				if (piece != EMPTY) {
					int i=0;
					p="X";
					for (byte validPiece: PIECES) {
						if (validPiece == piece) {
							p = VALID_FILES.substring(i, i+1);
							break;
						}
						i++;
					}
				}
				//System.out.print(p+"-"+file+" ");
				System.out.print(p+" ");
			}
		}
		System.out.println();
	 ...
	 *
	 * @return A byte representing the colour of the player whos turn it is
	 *         currently.
	 */
	 ...
	 *
	 * @return True if the destination square index is on the playable chess board,
	 *         false otherwise.
	 */
	private boolean isValidDestination(int destination) {
		return (((destination & 0x88) == VALID) && (squareEmpty(destination) || (pieceColourAt(destination) != this.turnColour)));
	}
	...
	/**
	 * Obtain a list of all valid moves that can be played on the current board
	 * position.
	 *
	...
	/**
	 * Generate all valid moves for the piece of type 'pieceType' located on square
	 * 'position'.
	 *
	...
	/**
	 * Generate all destinations for the piece of type 'pieceType' located on square
	 * 'position'.
	 *
	 * @param pieceType The type of the piece to generate moves for.
	 * @param position  The index of the current square position of the piece.
	...
	private boolean canCastleKingSide(int position) {
		return (!hasPieceMoved(pieceAt(position)) && !kingInCheck() && pieceTypeAt(position + 3) == ROOK && !hasPieceMoved(pieceAt(position + 3)) && squareEmpty(position + 1)
				&& !squareAttacked(position + 1) && squareEmpty(position + 2) && !squareAttacked(position + 2));
	}
	...
	private boolean canCastleQueenSide(int position) {
		return (!hasPieceMoved(pieceAt(position)) && !kingInCheck() && pieceTypeAt(position - 4) == ROOK && !hasPieceMoved(pieceAt(position - 4)) && squareEmpty(position - 1)
				&& !squareAttacked(position - 1) && squareEmpty(position - 2) && !squareAttacked(position - 2) && squareEmpty(position - 3) && !squareAttacked(position - 3));
	}
	 ...
	 *
	 * @return True if an opponents piece is located at that position, false
	 *         otherwise.
	 */
	 ...
	 *
	 * @return A score representing how good the piece development of the current
	 *         player is.
	 */
